...
export function resolve_tfolder(folder_str: string): TFolder {
    ...
    if (!folder) {
        throw new ElementsError(`Folder "${folder_str}" doesn't exist`);
    }
    if (!(folder instanceof TFolder)) {
        throw new ElementsError(`${folder_str} is a file, not a folder`);
    }
...
export function resolve_tfile(file_str: string): TFile {
    ...
    if (!file) {
        throw new ElementsError(`File "${file_str}" doesn't exist`);
    }
    if (!(file instanceof TFile)) {
        throw new ElementsError(`${file_str} is a folder, not a file`);
    }
