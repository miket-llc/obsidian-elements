...
/**
 * @type{logLevel} is a string type that represents a message level. It can also be used to set
...
export class WinstonLogger implements Logger {
	...
	private _fileTransportMap = new Map<
		string,
	 ...
	 */
	get level(): LogLevel {
		return this._consoleTranport.level as LogLevel;
	}
	...
	set level(level: LogLevel) {
		this._consoleTranport.level = level;
	}
	...
	removeFileTransport(filename: string): boolean {
		return (
			this._logger.remove(this._fileTransportMap.get(filename)) !==
			undefined
		);
	}
